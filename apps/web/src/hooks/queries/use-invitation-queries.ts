...
import type { CreateInvitationInput } from '@betaversionio/shared';
...
export function useReceivedInvitations() {
  return useQuery({
    ...
    queryFn: () => apiClient.get<ProjectInvitation[]>('/users/me/invitations'),
  });
}
...
export function useProjectInvitations(projectId: string) {
  return useQuery({
    ...
    queryFn: () =>
      apiClient.get<ProjectInvitation[]>(`/projects/${projectId}/invitations`),
    enabled: !!projectId,
  });
}
