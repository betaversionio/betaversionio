    ...
    >
      <ReactMarkdown
        ...
        components={{
          ...
          h3: ({ children }) => (
            <h3
              className={cn('scroll-m-20 font-semibold tracking-tight', s.h3)}
            >
              {children}
            </h3>
          ),
          h4: ({ children }) => (
            <h4
              className={cn('scroll-m-20 font-semibold tracking-tight', s.h4)}
            >
              {children}
            </h4>
          ),
          p: ({ children }) => (
            <p className={cn('text-muted-foreground', s.p)}>{children}</p>
          ),
          ul: ({ children }) => (
            <ul className={cn('ml-6 list-disc [&>li]:mt-2', s.ul)}>
              {children}
            </ul>
          ),
          ol: ({ children }) => (
            <ol className={cn('ml-6 list-decimal [&>li]:mt-2', s.ol)}>
              {children}
            </ol>
          ),
          li: ({ children }) => (
          ...
          code: ({ children }) => (
            <code
              className={cn(
                'relative rounded bg-muted px-[0.3rem] py-[0.2rem] font-mono text-muted-foreground',
                s.code,
              )}
            >
              {children}
