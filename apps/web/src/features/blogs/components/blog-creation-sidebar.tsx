...
export type BlogFormTab = 'main' | 'cover';

interface BlogCreationSidebarProps {
  activeTab: BlogFormTab;
  onTabChange: (tab: BlogFormTab) => void;
  onSubmit: () => void;
...
const tabs = [
  ...
  { id: 'cover' as const, label: 'Cover Image', icon: Gallery },
];

export function BlogCreationSidebar(props: BlogCreationSidebarProps) {
  return (
    <SidebarShell>
      <BackToBlogs />
  ...
  const link = (
    ...
    >
      <ArrowLeft2
        ...
        className="shrink-0"
      />
      {!collapsed && <span>Back to Blogs</span>}
    </Link>
  );
