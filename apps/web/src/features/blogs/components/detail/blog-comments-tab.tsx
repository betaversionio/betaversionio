...
import type { BlogComment } from '@/hooks/queries/use-blog-queries';
import {
  useBlogComments,
...
function BlogCommentItem({
  ...
  const updateComment = useUpdateBlogComment(blogId);
  const deleteComment = useDeleteBlogComment(blogId);
...
export function BlogCommentComposer({ blogId }: { blogId: string }) {
  const { user } = useAuth();
  const createComment = useCreateBlogComment(blogId);
...
interface BlogCommentsTabProps {
  blogId: string;
}

export function BlogCommentsTab({ blogId }: BlogCommentsTabProps) {
  const [page, setPage] = useState(1);
  const { data, isLoading } = useBlogComments(blogId, page);
