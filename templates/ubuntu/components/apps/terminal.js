...
export class Terminal extends Component {
    ...
    loadPortfolioData() {
        // Access portfolio data from window if available (set by context bridge)
        // Or build from props
        if (this.props.portfolioData) {
            const { user, projects } = this.props.portfolioData;
            this.username = user.username || 'user';
            this.hostname = 'ubuntu';

            const skills = (user.techStack || []).map(t => t.name);
            const projectNames = (projects || []).map(p => p.slug || p.title.toLowerCase().replace(/\s+/g, '-'));
            const interests = (user.services || []).map(s => s.title);

            this.child_directories = {
                root: ["projects", "skills", ...(interests.length > 0 ? ["interests"] : [])],
                skills: skills,
    ...
    componentDidUpdate(prevProps) {
        ...
        if (prevProps.portfolioData !== this.props.portfolioData && this.props.portfolioData) {
            this.loadPortfolioData();
        }
    }
        ...
        return (
            <React.Fragment key={id}>
                <div className="flex w-full h-5">
                    <div className="flex">
                        <div className=" text-ubt-green">{this.username}@{this.hostname}</div>
    ...
    handleCommands = (command, rowId) => {
        ...
        switch (main) {
            ...
            case "pwd":
                ...
                result = str.replace("~", `/home/${this.username}`)
                break;
            case "code":
                if (words[0] === "." || words.length === 0) {
                    this.props.openApp("vscode");
                } else {
                    result = "Command '" + main + "' not found, or not yet implemented.<br>Available Commands: [ cd, ls, pwd, echo, clear, exit, mkdir, code, spotify, chrome, about-me, trash, settings, sendmsg ]";
                }
                break;
            case "echo":
            ...
            case "exit":
                ...
                return;
            case "sudo":
                result = "Nice try! But you don't have sudo access here.";
                break;
            default:
                result = "Command '" + main + "' not found, or not yet implemented.<br>Available Commands: [ cd, ls, pwd, echo, clear, exit, mkdir, code, spotify, chrome, about-me, trash, settings, sendmsg ]";
        }
